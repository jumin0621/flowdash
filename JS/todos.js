...
function makeTask(todo) {
  ...
  ${completedAtHtml}`;
    ...
    <p class="task-title" style="text-decoration:${doneLine};">
      ${todo.title}
    </p>

    <p class="task-desc" style="text-decoration:${doneLine};">
      ${todo.content || ""}
    </p>
...
export function render() {
	...
	// 상태별로 나눠서 추가
	const todoList = viewTodos.filter((t) => t.status === "todo"); //todos에서 viewTodos로 바꿈
	const doingList = viewTodos.filter((t) => t.status === "doing"); //todos에서 viewTodos로 바꿈
	const doneList = viewTodos.filter((t) => t.status === "done"); //todos에서 viewTodos로 바꿈
